...
namespace IPOWeb.Controllers
{
    public class AuditReportController : Controller
    {
        ...
        public AuditReportController(IConfiguration configuration)
        {
        ...
        [HttpGet]
        public JsonResult GetAuditReportTrail(string list,string sdate, string edate)
        {
            urlstring = Convert.ToString(_configuration.GetSection("Appsettings")["apiurl"]) + "GetAuditReportTrail";
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    string url = urlstring + "?&list=" + list + "&sdate=" + sdate + "&edate=" +edate;
