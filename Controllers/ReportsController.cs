...
using System.Text;
using static IPOWeb.Controllers.CommonController;
using static IPOWeb.Controllers.ReportsController.ResultSetModel;
        ...
        public JsonResult generatedynamicreport([FromBody] generatedynamicReportmodel context)
        {
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    Console.WriteLine("Auth Header: Bearer " + token);
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    HttpContent content = new StringContent(JsonConvert.SerializeObject(context), UTF8Encoding.UTF8, "application/json");
                    try
